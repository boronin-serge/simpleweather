 ...
 */
class DetailedWeatherMapperImpl(
  ...
  override fun map(data: DetailedWeatherResponse): DetailedForecastModel {
    ...
    return DetailedForecastModel(
      ...
      weatherByDays.values.map { listByHours ->
        countWeatherType(listByHours)
        val weather = listByHours.maxByOrNull { it.weatherType }!!.weatherType
        countIconType(listByHours)
        val icon = listByHours.maxByOrNull { it.iconId }!!.iconId
        DayForecastModel(
          listByHours.minByOrNull { forecastModel -> forecastModel.temperature }?.temperature
            ?: DEFAULT_INT,
          listByHours.maxByOrNull { forecastModel -> forecastModel.temperature }?.temperature
            ?: DEFAULT_INT,
